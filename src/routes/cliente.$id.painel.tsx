...
function PainelCliente() {
  ...
  const pieData = Object.entries(counts)
    .filter(([, v]) => v > 0)
    .map(([k, v]) => ({
      name: k === "VIGENTE" ? "Vigente" : k === "PROXIMO" ? "Próximo do vencimento" : k === "RISCO" ? "Em risco" : "Vencido",
      value: v, key: k,
    }));
      ...
      <div className="min-h-screen" style={{ background: "#fff", fontFamily: "system-ui, sans-serif" }}>
        <div className="max-w-3xl mx-auto px-8 py-10">
          ...
          <div className="flex items-start justify-between mb-8">
            <div>
              <div className="text-[11px] font-semibold tracking-widest uppercase mb-1" style={{ color: "#64748b" }}>Arbrent Contabilidade</div>
              <h1 className="text-2xl font-bold" style={{ color: "#0f172a" }}>{empresa.razaoSocial}</h1>
              <p className="text-[13px] mt-0.5" style={{ color: "#64748b" }}>Relatório de Contratos de Experiência</p>
            </div>
            <div className="text-right no-print">
              <p className="text-[12px] mb-3" style={{ color: "#64748b" }}>{hoje}</p>
            ...
            <div className="text-right" style={{ display: "none" }}>
              <p className="text-[12px]" style={{ color: "#64748b" }}>{hoje}</p>
            </div>
          </div>

          {/* Frase resumo */}
          <div className="mb-8 p-5 rounded-lg" style={{ background: "#f8fafc", borderLeft: "4px solid #071040" }}>
            <p className="text-[15px] leading-relaxed" style={{ color: "#0f172a" }}>
              {total === 0
                ? "Nenhum colaborador em contrato de experiência ativo."
                : <>
                    Você tem <strong>{total} colaborador{total !== 1 ? "es" : ""}</strong> em contrato de experiência.{" "}
                    {decisoes > 0
                      ? <><strong>{decisoes} decisão{decisoes !== 1 ? "ões precisam" : " precisa"} ser tomada{decisoes !== 1 ? "s" : ""}</strong> este mês.</>
              ...
              <ResponsiveContainer width="100%" height={200}>
                <PieChart>
                  ...
                  <Tooltip
                    ...
                    contentStyle={{ background: "#fff", border: "1px solid #e2e5f0", borderRadius: 6, fontSize: 12 }}
                  />
                </PieChart>
              </ResponsiveContainer>
          ...
          <div>
            <h2 className="text-[11px] font-semibold tracking-widest uppercase mb-3" style={{ color: "#64748b", borderBottom: "1px solid #e2e5f0", paddingBottom: 6 }}>
              Colaboradores
            </h2>
            {sorted.length === 0 ? (
              <p className="text-[13px]" style={{ color: "#64748b" }}>Nenhum colaborador com contrato ativo.</p>
            ) : (
              <div className="space-y-2">
                {sorted.map(({ c, info }) => {
                  const cor = SEMAFORO[info.status];
                  const alerta = info.diasRestantes <= 7;
                  const textoVenc = info.diasRestantes < 0
                  ...
                  return (
                    <div key={c.id} className="flex items-center gap-3 py-2" style={{ borderBottom: "1px solid #f1f5f9" }}>
                      <span className="w-2 h-2 rounded-full shrink-0" style={{ background: cor }} />
                      <span className="text-[13px] font-medium w-48 shrink-0" style={{ color: "#0f172a" }}>{c.funcionarioNome}</span>
                      <span className="text-[12px] flex-1" style={{ color: "#64748b" }}>{c.cargo}</span>
                      <span className="text-[12px]" style={{ color: cor, fontWeight: alerta ? 600 : 400 }}>{textoVenc}</span>
          ...
          {/* Rodapé */}
          <div className="mt-10 pt-4 text-[11px] text-center" style={{ borderTop: "1px solid #e2e5f0", color: "#94a3b8" }}>
