...
function useAtribuirAnalistaEmpresas() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async ({ analistaId, empresaIds }: { analistaId: string; empresaIds: string[] }) => {
  ...
  if (!isSupervisor) {
    return (
      <div>
        ...
        <Surface>
          <EmptyState icon="alert" title="Acesso restrito" subtitle="Apenas supervisores podem gerenciar usuários." />
        </Surface>
      </div>
    );
  }
  ...
  if (isLoading) {
    return (
      <div>
        ...
        <Surface><div className="py-8 text-center text-[13px]" style={{ color: "#94a3b8" }}>Buscando usuários…</div></Surface>
      </div>
    );
  }
    ...
    <div>
      ...
      <Surface>
        <table className="text-[12px] w-full">
          ...
          <tbody>
              ...
              return (
                <tr key={p.id} style={{ borderTop: "1px solid #e2e5f0" }}>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <div className="font-medium" style={{ color: "#0f172a" }}>
                          {p.nome}
                          {isMe && <span className="ml-1.5 text-[10px] px-1.5 py-0.5 rounded" style={{ background: "#e0e7ff", color: "#4338ca" }}>você</span>}
                        </div>
                      </div>
                    </div>
                  </Td>
                  ...
                  <Td>
                    {p.role === "analista" ? (
                      <span style={{ color: empCount > 0 ? "#0f172a" : "#94a3b8" }}>
                        {empCount > 0 ? `${empCount} empresa${empCount !== 1 ? "s" : ""}` : "Nenhuma"}
                      </span>
                    ) : (
                  ...
                  <Td>
                    <span
                      className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[11px]"
                      style={{ background: p.ativo ? "#22c55e22" : "#94a3b822", color: p.ativo ? "#16a34a" : "#475569" }}
                    >
                      <span className="w-1.5 h-1.5 rounded-full" style={{ background: p.ativo ? "#22c55e" : "#94a3b8" }} />
                  ...
                  <Td>
                    {!isMe && (
                      <div className="inline-flex items-center gap-1 p-1 rounded-lg" style={{ background: "#f8fafc", border: "1px solid #e2e5f0" }}>
                        ...
                        <button
                          onClick={async () => {
                            const newRole: UserRole = p.role === "supervisor" ? "analista" : "supervisor";
                            await alterarRole.mutateAsync({ id: p.id, role: newRole });
                            toast.success(`${p.nome} agora é ${newRole === "supervisor" ? "Supervisor" : "Analista"}`);
                          }}
                          className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-[11px] font-medium hover:bg-slate-100"
                          style={{ color: "#4338ca" }}
                        >
                          <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2M9 11a4 4 0 100-8 4 4 0 000 8zM23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75"/></svg>
                        ...
                        <button
                          ...
                          style={{ color: p.ativo ? "#d97706" : "#16a34a", background: p.ativo ? "#fffbeb" : "#f0fdf4" }}
                        >
            ...
            {profiles.length === 0 && (
              <tr><td colSpan={5}>
                <EmptyState icon="search" title="Nenhum usuário encontrado" subtitle="Convide alguém para começar." />
              </td></tr>
            )}
          </tbody>
        </table>
      ...
      {assignOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(0,0,0,0.4)" }} onClick={() => setAssignOpen(null)}>
          <div className="w-full max-w-md rounded-xl shadow-xl p-6 bg-white" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-[15px] font-semibold mb-1" style={{ color: "#0f172a" }}>Atribuir empresas</h2>
            ...
            <div className="space-y-2 max-h-64 overflow-y-auto mb-4">
              {empresas.map((e) => (
                <label key={e.id} className="flex items-center gap-2.5 cursor-pointer">
                  ...
                  />
                  <span className="text-[13px]" style={{ color: "#0f172a" }}>{e.nomeFantasia}</span>
                  <span className="text-[11px]" style={{ color: "#64748b" }}>{e.cnpj}</span>
                </label>
              ))}
            </div>
            <div className="flex justify-end gap-2">
              <button onClick={() => setAssignOpen(null)} className="px-4 py-2 rounded-md text-[13px]" style={{ background: "#f1f5f9", color: "#0f172a" }}>Cancelar</button>
              <button onClick={saveAssign} className="px-4 py-2 rounded-md text-[13px] font-medium text-white" style={{ background: "#071040" }}>Salvar</button>
            </div>
          </div>
        </div>
      )}
...
function InviteModal({ onClose }: { onClose: () => void }) {
  ...
  const handleInvite = async () => {
    const e = email.trim();
    const n = nome.trim();
    if (!e || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e)) { setError("E-mail inválido."); return; }
    if (!n) { setError("Nome obrigatório."); return; }
    setError(null);
    setLoading(true);
    try {
        ...
        {sent ? (
          <>
            <div className="text-center py-4">
              ...
              <h2 className="text-[15px] font-semibold mb-1" style={{ color: "#0f172a" }}>Convite enviado!</h2>
              <p className="text-[12px]" style={{ color: "#64748b" }}>
                Um e-mail de confirmação foi enviado para <strong>{email}</strong>.<br />
                O usuário deve confirmar o e-mail para acessar o sistema.
              </p>
            </div>
            <div className="flex justify-center mt-4">
              <button onClick={onClose} className="px-4 py-2 rounded-md text-[13px] font-medium text-white" style={{ background: "#071040" }}>Fechar</button>
            </div>
          </>
        ) : (
          <>
            <h2 className="text-[15px] font-semibold mb-1" style={{ color: "#0f172a" }}>Convidar usuário</h2>
            <p className="text-[12px] mb-4" style={{ color: "#64748b" }}>O usuário receberá um e-mail para confirmar o cadastro.</p>
            <div className="space-y-3">
              <div>
                <label className="block text-[12px] font-medium mb-1" style={{ color: "#374151" }}>Nome completo</label>
                <input
                  value={nome}
              ...
              <div>
                <label className="block text-[12px] font-medium mb-1" style={{ color: "#374151" }}>E-mail</label>
                <input
                  type="email"
                  value={email}
              ...
              <div>
                <label className="block text-[12px] font-medium mb-1" style={{ color: "#374151" }}>Papel</label>
              ...
              {error && <div className="text-[12px] px-3 py-2 rounded-md" style={{ background: "#fef2f2", color: "#dc2626", border: "1px solid #fecaca" }}>{error}</div>}
            </div>
            <div className="flex justify-end gap-2 mt-4">
              <button onClick={onClose} className="px-4 py-2 rounded-md text-[13px]" style={{ background: "#f1f5f9", color: "#0f172a" }}>Cancelar</button>
              <button
