...
import {
  ...
  useRemoveColaborador,
  useToggleAtivoColaborador,
...
export const Route = createFileRoute("/empresas_/$id")({
  ...
  notFoundComponent: () => (
    <div className="text-center py-16">
      ...
      <Link to="/empresas" className="text-[13px]" style={{ color: "#4f8ef7" }}>
...
function EmpresaDetail() {
  ...
  const filteredColaboradores = useMemo(() => {
    ...
    return colaboradores.filter(
      (c) =>
        c.nome.toLowerCase().includes(q) || c.cargo.toLowerCase().includes(q) || c.cpf.includes(q),
    );
  ...
  const renovados = useMemo(() => {
    ...
    return allContratos.filter((c) => {
      ...
      return (
        isAfter(startOfDay(parseISO(c.renovadoEm)), corte) ||
        startOfDay(parseISO(c.renovadoEm)).getTime() === corte.getTime()
      );
    });
  ...
  const handleEncerrar = async () => {
    ...
    toast.success(
      `${selected.length} contrato${selected.length !== 1 ? "s" : ""} encerrado${selected.length !== 1 ? "s" : ""}`,
    );
    setSelected([]);
  };
  ...
  const handleSaveColaborador = async (col: Colaborador) => {
    ...
    setEditing(null);
    toast.success(
      isNew ? `Colaborador "${col.nome}" adicionado` : `Colaborador "${col.nome}" atualizado`,
    );
  };
      ...
      <Surface className="mb-4">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-[12px]">
          ...
          <InfoField
            ...
            color={empresa.ativo ? "#16a34a" : "#475569"}
          />
        </div>
      </Surface>
        ...
        <div className="flex items-center justify-between gap-3 mb-4 flex-wrap">
          <div>
            ...
            <p className="text-[11px] mt-0.5" style={{ color: "#64748b" }}>
              ...
              {colaboradores.filter((c) => c.ativo).length} ativo
              {colaboradores.filter((c) => c.ativo).length !== 1 ? "s" : ""}
            </p>
          </div>
          <button
            onClick={() => {
        ...
        {colaboradores.length > 0 && (
          <div className="flex items-center gap-3 mb-3">
            ...
            <span className="text-[11px]" style={{ color: "#64748b" }}>
              {filteredColaboradores.length} resultado
              ...
              <tbody>
                {filteredColaboradores.map((col) => (
                  <tr key={col.id} style={{ borderTop: "1px solid #e2e5f0" }}>
                    <ColTd>
                      <div className="font-medium" style={{ color: "#071040" }}>
                        {col.nome}
                      </div>
                      ...
                      {col.observacao && (
                        <div
                          ...
                          style={{ color: "#94a3b8" }}
                          title={col.observacao}
                        >
                          {col.observacao.length > 40
                            ? col.observacao.slice(0, 40) + "…"
                            : col.observacao}
                        </div>
                      )}
                    </ColTd>
                    ...
                    <ColTd>
                      {confirmColId === col.id ? (
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <button
                            onClick={async () => {
                              await toggleAtivoColaborador.mutateAsync({
                                id: col.id,
                                ativo: !col.ativo,
                              });
                              toast.success(
                                col.ativo ? `${col.nome} desativado` : `${col.nome} ativado`,
                              );
                            }}
        ...
        <div className="flex items-start justify-between gap-3 mb-3 flex-wrap">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span
              ...
              style={{
                background: renovados.length > 0 ? "#dbeafe" : "#f1f5f9",
                color: renovados.length > 0 ? "#1d4ed8" : "#94a3b8",
              }}
            >
              ...
              <tbody>
                {renovados.map((c) => (
                  <tr key={c.id} style={{ borderTop: "1px solid #e2e5f0" }}>
                    ...
                    <ColTd>
                      {format(parseISO(c.vencimentoSegundo), "dd/MM/yyyy", { locale: ptBR })}
                    </ColTd>
                  </tr>
                ))}
              </tbody>
      ...
      <Surface title="Contratos">
        ...
        {selected.length > 0 && (
          <div
            ...
            style={{ background: "#f0f5ff", border: "1px solid #c7d2fe" }}
          >
            <span className="text-[12px]" style={{ color: "#071040" }}>
...
function InfoField({ label, value, color }: { label: string; value: string; color?: string }) {
  return (
    <div>
      <div className="uppercase text-[10px] tracking-wider" style={{ color: "#64748b" }}>
        {label}
      </div>
      <div className="mt-1 text-[13px] font-medium" style={{ color: color ?? "#0f172a" }}>
        {value}
      </div>
    </div>
  );
