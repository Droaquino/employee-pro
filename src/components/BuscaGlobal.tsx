  ...
  const results = useMemo<Result[]>(() => {
    ...
    if (out.length < 8) {
      for (const e of empresas) {
        if (!e.nomeFantasia.toLowerCase().includes(q) && !e.razaoSocial.toLowerCase().includes(q)) continue;
        const total = contratos.filter((c) => c.empresaId === e.id && !c.encerrado).length;
        const emRisco = contratos.filter((c) => c.empresaId === e.id && !c.encerrado && ["RISCO", "VENCIDO"].includes(calcStatus(c).status)).length;
        out.push({
          type: "empresa",
          id: e.id,
          nome: e.nomeFantasia,
      ...
      >
        <div className="flex items-center gap-3 px-4 py-3" style={{ borderBottom: "1px solid #f1f5f9" }}>
          ...
          <input
            ...
            style={{ color: "#0f172a" }}
          />
          <kbd className="text-[10px] px-1.5 py-0.5 rounded" style={{ background: "#f1f5f9", color: "#64748b", border: "1px solid #e2e5f0" }}>ESC</kbd>
        </div>

        {query.length >= 2 && (
          <div className="max-h-[360px] overflow-y-auto">
            {results.length === 0 ? (
              <div className="px-4 py-6 text-center text-[13px]" style={{ color: "#94a3b8" }}>Nenhum resultado encontrado</div>
            ) : (
              results.map((r, i) => (
                <button
                  key={r.id + i}
                ...
                >
                  {r.type === "funcionario" ? (
                    <>
                      <span className="w-2 h-2 rounded-full shrink-0" style={{ background: STATUS_COLOR[r.status] }} />
                      <div className="flex-1 min-w-0">
                        <div className="text-[13px] font-medium truncate" style={{ color: "#0f172a" }}>{r.nome}</div>
                        <div className="text-[11px]" style={{ color: "#64748b" }}>{r.empresa} · {STATUS_LABEL[r.status]}</div>
                      </div>
                      <span className="text-[11px] shrink-0" style={{ color: STATUS_COLOR[r.status] }}>
                        {r.diasRestantes < 0 ? `vencido` : r.diasRestantes === 0 ? "hoje" : `${r.diasRestantes}d`}
                      </span>
                    </>
                  ) : (
                    <>
                      <span className="w-6 h-6 rounded shrink-0 flex items-center justify-center text-[10px] font-bold text-white" style={{ background: "#071040" }}>
                        {r.nome.charAt(0)}
                      </span>
                      <div className="flex-1 min-w-0">
                        <div className="text-[13px] font-medium truncate" style={{ color: "#0f172a" }}>{r.nome}</div>
                        <div className="text-[11px]" style={{ color: "#64748b" }}>{r.total} contrato{r.total !== 1 ? "s" : ""} · {r.situacao}</div>
                      </div>
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#cbd5e1" strokeWidth="2"><path d="m9 18 6-6-6-6"/></svg>
                    </>
                  )}
                </button>
              ))
