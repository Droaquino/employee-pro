...
export function TimelineGantt({ contratos }: { contratos: Contrato[] }) {
  ...
  const data = contratos
    ...
    .map((c) => {
      ...
      return {
        nome:
          c.funcionarioNome.length > 18 ? c.funcionarioNome.slice(0, 17) + "…" : c.funcionarioNome,
        offset: start,
        length,
        cargo: c.cargo,
      ...
      >
        ...
        <Tooltip
          ...
          formatter={(_v: any, _n: any, p: any) => [
            `${p.payload.cargo} · ${p.payload.admissao} → ${p.payload.vencimento} (${p.payload.diasRestantes}d)`,
            "Contrato",
          ]}
        />
