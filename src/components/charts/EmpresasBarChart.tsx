...
export function EmpresasBarChart({ empresas, contratos }: { empresas: Empresa[]; contratos: Contrato[] }) {
  ...
  return (
    <ResponsiveContainer width="100%" height={300}>
      <BarChart data={visible} layout="vertical" margin={{ left: 10, right: 16, top: 4, bottom: 4 }}>
        ...
        <YAxis dataKey="nome" type="category" width={120} tick={{ fontSize: 11, fill: COLORS.textPrimary }} />
        <Tooltip contentStyle={{ background: "#fff", border: `1px solid ${COLORS.borderSoft}`, borderRadius: 6, fontSize: 12 }} />
        <Legend wrapperStyle={{ fontSize: 11 }} />
        <Bar dataKey="VIGENTE" stackId="a" fill={STATUS_COLOR.VIGENTE} name="Vigente" onClick={(d: any) => d.empresaId && navigate({ to: "/empresas/$id", params: { id: d.empresaId } })} cursor="pointer" />
        <Bar dataKey="PROXIMO" stackId="a" fill={STATUS_COLOR.PROXIMO} name="Próximo" onClick={(d: any) => d.empresaId && navigate({ to: "/empresas/$id", params: { id: d.empresaId } })} cursor="pointer" />
        <Bar dataKey="RISCO" stackId="a" fill={STATUS_COLOR.RISCO} name="Em risco" onClick={(d: any) => d.empresaId && navigate({ to: "/empresas/$id", params: { id: d.empresaId } })} cursor="pointer" />
        <Bar dataKey="VENCIDO" stackId="a" fill={STATUS_COLOR.VENCIDO} name="Vencido" onClick={(d: any) => d.empresaId && navigate({ to: "/empresas/$id", params: { id: d.empresaId } })} cursor="pointer" />
      </BarChart>
    </ResponsiveContainer>
  );
}
