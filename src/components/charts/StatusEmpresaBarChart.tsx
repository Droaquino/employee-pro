    ...
    <div>
      <ResponsiveContainer width="100%" height={200}>
        <BarChart data={data} margin={{ top: 8, right: 16, bottom: 4, left: 0 }}>
          ...
          <Tooltip
            formatter={(value: number, _name: string, props: any) => [`${value} contrato${value !== 1 ? "s" : ""}`, props.payload?.label ?? ""]}
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-2 justify-center mt-2">
        {data.map((d) => (
          <div key={d.key} className="flex items-center gap-1.5 text-[11px]" style={{ color: COLORS.textPrimary }}>
            <span style={{ width: 10, height: 10, borderRadius: 3, background: STATUS_COLOR[d.key], flexShrink: 0, display: "inline-block" }} />
            <span>{d.label}</span>
